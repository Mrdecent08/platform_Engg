...
update_deployment_resources() {
  ...
  if [[ "$current_cpu_request" == "null" ]]; then
...
new_mem_request=$(increase_resource $current_mem_request 20)
new_mem_limit=$(increase_resource $current_mem_limit 20)



# Update the deployment with new resource requests and limits
update_deployment_resources $current_cpu_request $new_mem_request $current_cpu_limit $new_mem_limit
